...
(Heavily modified by Daven Quinn, USGS, 2011)
    ...
    class ImportWizardPanel_Delimiters(wx.Panel):  # change DQ
        ...
        ARGUMENTS:
          ...
          - isValidCallback is a callback function that accepts a single boolean argument
            ...
            exception.
          - field_choices is a list of potential field definitions that can be used. Only one of each field definition will be allowed
        METHODS:
          - GetDelimiters()
        ...
        def __init__(
            ...
            if self.field_choices != None:
                self.BuildPreviewFields()
                previewStaticSizer.Add(self.preview_fields, 0, wx.ALL | wx.EXPAND, 5)
        ...
        def BuildPreviewFields(self):
            ...
            fds = self.preview.GetNumberCols()
            self.preview_fields.CreateGrid(1, fds)
            ...
            for field in range(fds):
                ...
                self.preview_fields.SetColSize(field, w)

            rowheight = self.preview_fields.GetRowSize(0) + 4
            self.preview_fields.SetSize((-1, rowheight))

        def GetPreviewFields(self):
            fields = dict()
            for field in range(self.preview_fields.GetNumberCols()):
                name = self.preview_fields.GetCellValue(0, field)
                fields[name] = field
            return fields
