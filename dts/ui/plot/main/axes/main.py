...
class MainPlot(AxesBase):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.image = self.imshow(
            self.figure.parent.array,
            aspect="auto",
    ...
    def _distance_ticks(self, interval=None, offset=None):
        ...
        log.debug(
            "Setting distance ticks: Interval: {0}, Offset: {1}".format(
                interval, offset
            )
        )
    ...
    def _set_distance_ticks(self, event=None):
        ...
        distances = self._distance_ticks(interval, offset)
        ...
        formatter = matplotlib.ticker.FuncFormatter(format_distance)
    ...
    def _set_time_ticks(self, event=None):
        locator = matplotlib.ticker.MaxNLocator(nbins=8, prune="both")
        ...
        def format_time(y, pos):
            idx = int(round(y))
            if 0 <= idx < len(times):
                dt = datetime.datetime.fromtimestamp(times[idx], tz=tzlocal())
                return dt.strftime(time_format.replace("%f", "000000"))
            return ""
