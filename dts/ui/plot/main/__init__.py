...
class MainViewer(PlotPanel):
    ...
    def __init__(self, parent, dataset, id=wx.ID_ANY, dpi=None, **kwargs):
        ...
        PlotPanel.__init__(
            self, parent, id=id, dpi=None, toolbar=MainPlotToolbar, **kwargs
        )
    ...
    def _onCheck(self, evt):
        for key, item in self.toolbar.params.items():
            if item.GetId() == evt.GetId():
                self.c_flags[key] = evt.IsChecked()
    ...
    def _onTooltipMoveEvent(self, event):
        if event.source is self:
            ...
            try:
                log.debug(
                    "Moving tooltip to {},{} in {} showing {}".format(
                        self.x_loc,
                        self.t_loc,
                ...
                log.debug(
                    "Clearing tooltip in {} showing {}".format(
                        self.__class__.__name__, self.data.array.name
                    )
                )
                self.clear_tooltip()
    ...
    def _onclick(self, event):
        if event.inaxes is self.main:
            ...
            log.debug(
                "Click in {} at {}, {}".format("main axes", self.x_loc, self.t_loc)
            )
    ...
    def _set_temp_extents(self, min=None, max=None):
        ...
        log.debug(
            "Adjusting temp_extents in {} showing {} to {}, {}".format(
                self.__class__.__name__, self.data.get_title(), min, max
            )
        )
    ...
    def __fireMoveTooltipEvent(self, event=None):
        ...
        log.debug(
            "Firing MoveTooltipMovedEvent for {} showing {}".format(
                self.__class__.__name__, self.data.array.name
            )
        )
    ...
    def main_plot(self, shared=True):
        if "main" in self.__dict__:
            ...
            if shared:
                ...
                opts = dict()
            self.main = self.figure.add_subplot(
                self.grid[2], projection="MainPlot", **opts
            )
