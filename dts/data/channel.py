...
class Channel(object):
    ...
    def trim_raw(self, x_min=None, x_max=None, t_min=None, t_max=None):
        ...
        if x_min is None:
            x_min = 0
        if x_max is None:
            x_max = data_raw.shape[1] - 1
        if t_min is None:
            t_min = 0
        if t_max is None:
            t_max = data_raw.shape[0] - 1
        ...
        del self.__hdf__[
            "data"
        ]  # must delete old copy of dataset before creating a new one.
        data = self.__hdf__.create_dataset(
            "data", data=data_raw[t_min:t_max, x_min:x_max]
        )
